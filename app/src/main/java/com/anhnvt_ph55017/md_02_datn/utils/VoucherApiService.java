...
public class VoucherApiService {
    ...
    public static void getMyCreatedVouchers(Context context, VoucherListCallback callback) {
        new Thread(() -> {
            try {
                java.net.URL url = new java.net.URL(NetworkConstants.API_BASE_URL + "/api/vouchers/shop/my");
                java.net.HttpURLConnection conn = (java.net.HttpURLConnection) url.openConnection();
                conn.setRequestMethod("GET");
                conn.setRequestProperty("Authorization", "Bearer " + SessionManager.getToken(context));
                int responseCode = conn.getResponseCode();
                ...
                String response = scanner.hasNext() ? scanner.next() : "";
                scanner.close();
                android.util.Log.d("VOUCHER_API", "ShopMyVouchers Response: " + response);
                if (responseCode >= 200 && responseCode < 300) {
                    List<Voucher> vouchers = new ArrayList<>();
                    try {
                        JSONObject obj = new JSONObject(response);
                        ...
                        android.util.Log.e("VOUCHER_API", "Parse error (shop vouchers): " + e.getMessage(), e);
                    }
                    android.util.Log.e("VOUCHER_API", "Không tìm thấy danh sách voucher shop trong response!");
                    callback.onError("Không tìm thấy danh sách voucher shop trong response!");
                } else {
                    android.util.Log.e("VOUCHER_API", "Shop voucher API error response: " + response);
                    callback.onError(response);
                }
            } catch (Exception e) {
                android.util.Log.e("VOUCHER_API", "Exception: " + e.getMessage(), e);
