    ...
    order.items.forEach((item) => {
      ...
      if (isFeeable) {
        ...
        const feeAmount = roundCurrency(
          baseAmount * SHOP_BILLING_POLICY.commissionRate,
        );
        ...
        item.platformFee.baseAmount = roundCurrency(
...
const updateVoucherUsageAfterOrder = async (order) => {
  if (!order?.discount?.code) return;
  ...
  const hasUsageLimit =
    Number.isFinite(voucher.usageLimit) && voucher.usageLimit > 0;
  const isExhausted = hasUsageLimit && voucher.usedCount >= voucher.usageLimit;
...
exports.updateVoucherUsageAfterOrder = updateVoucherUsageAfterOrder;
...
const settlePaymentAndCreditShops = async (orderId) => {
  try {
    ...
    for (const item of order.items) {
      ...
      const feeAmount = roundCurrency(
        item.platformFee?.feeAmount ||
          (item.price || 0) * qty * SHOP_BILLING_POLICY.commissionRate,
      );
    ...
    for (const [shopId, agg] of shopMap.entries()) {
      ...
      shop.shopWallet.balance = roundCurrency(
        (shop.shopWallet.balance || 0) + net,
      );
...
const buildOrderResponse = (order) => {
  ...
  const discountAmount =
    orderObj.discount?.amount || orderObj.discount?.value || 0;
  ...
  const cancellationReason =
    orderObj.cancellationReason || orderObj.reason || "";
...
exports.createOrder = async (req, res) => {
  try {
    ...
    const voucherCodeValue =
      voucherCode ||
      discountCode ||
      (discountBody && discountBody.code) ||
      null;
    ...
    if (!name || !phone || !address || !city || !district || !ward) {
      return res
        .status(400)
        .json({ message: "Thông tin giao hàng không hợp lệ" });
    }
    ...
    if (/\d/.test(city)) {
      return res
        .status(400)
        .json({ message: "Tỉnh/Thành phố không được chứa số" });
    }

    const productIds = items.map((item) => item.product);
    const productsInDb = await Product.find({
      _id: { $in: productIds },
    }).select("_id shopId createdAt billing name stock");
    ...
    const shopIds = [
      ...new Set(productsInDb.map((product) => product.shopId.toString())),
    ];
    const shops = await User.find({ _id: { $in: shopIds } }).select(
      "_id shopStatus",
    );
    const frozenShops = new Set(
      shops
        .filter((shop) => shop.shopStatus === "frozen")
        .map((shop) => shop._id.toString()),
    );
    const blockedProduct = productsInDb.find((product) =>
      frozenShops.has(product.shopId.toString()),
    );
    if (blockedProduct) {
      return res.status(400).json({
        message: `Sản phẩm ${blockedProduct.name} hiện không thể bán do shop đang bị đóng băng`,
      });
    }
    ...
    let finalDiscountType =
      discountBody && typeof discountBody === "object"
        ? discountBody.type || discountBody?.type
        : undefined;
    ...
    if (voucherCodeValue) {
      ...
      if (voucherFromDb) {
        ...
        if (
          voucherFromDb.userLimit &&
          (claimedVoucher.usedCount || 0) >= voucherFromDb.userLimit
        ) {
          return res
            .status(400)
            .json({ message: "Bạn đã dùng voucher này tối đa số lần" });
        }
    ...
    const orderData = {
      ...
      total:
        typeof total === "number"
          ? total
          : roundCurrency(
              (subtotal || 0) - finalDiscountAmount + (shipping.fee || 0),
            ),
      discount: {
    ...
    try {
      await Promise.all(
        order.items.map(async (item) => {
          ...
          if (!updatedProduct) {
            throw new Error(
              `Sản phẩm ${productMap[String(item.product)]?.name || "không đủ"} không còn đủ hàng`,
            );
          }
        }),
      );
      ...
      return res.status(400).json({
        message: stockErr.message || "Không đủ tồn kho để tạo đơn hàng",
      });
    }

    if (voucherFromDb && claimedVoucher && voucherUser) {
      await updateVoucherUsageAfterOrder(order);
    }

    // Tạo thông báo cho shop (mỗi shop một thông báo)
    for (const shopId of shopIds) {
      const relatedCount = order.items
        .filter((item) => item.shopId.toString() === shopId)
  ...
  try {
    ...
    if (method === "card") {
      if (
        !cardData ||
        !cardData.cardNumber ||
        !/^\d{16}$/.test(cardData.cardNumber)
      ) {
        return res
          .status(400)
          .json({ message: "Số thẻ không hợp lệ, cần 16 chữ số" });
      }
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    if (order.discount?.code) {
      try {
        const voucherFromDb = await Voucher.findOne({
        ...
        if (voucherFromDb && user) {
          const userVoucherEntry = user.userVouchers.find(
            (entry) =>
              entry.voucher &&
              entry.voucher._id.toString() === voucherFromDb._id.toString(),
          );

          if (userVoucherEntry && userVoucherEntry.usedCount > 0) {
            // Giảm usedCount và nếu chưa dùng hết thì mở lại voucher
            userVoucherEntry.usedCount = Math.max(
              0,
              userVoucherEntry.usedCount - 1,
            );
            if (
              voucherFromDb.userLimit &&
              userVoucherEntry.usedCount < voucherFromDb.userLimit
            ) {
